...
@section('content')
        ...
        <table class="w-[1350px] rounded-[10px] overflow-hidden">
            ...
            <tbody>
                @foreach ($faculties as $index => $faculty)
                    <tr>
                        <td class="text-center text-[24px] w-[120px]">{{$index + 1}}</td>
                        <td class="text-[24px] text-left pl-4 flex flex-col">
                            ...
                            <a href="" class="showBtn text-[14px] text-[#5191FF] hover:underline w-[85px]">แสดงหลักสูตร</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <script>
        ...
        document.querySelectorAll(".showBtn").forEach((btn, index) => {
            btn.addEventListener("click", function (e) {
                e.preventDefault();
                const row = btn.closest("tr"); // หาแถวปัจจุบัน
                const myBtn = row.querySelector("#myBtn");
                const showText = row.querySelector("#showText");

                if (myBtn) myBtn.hidden = !myBtn.hidden;
                if (showText) showText.hidden = !showText.hidden;
            });
        });
    </script>
@endsection