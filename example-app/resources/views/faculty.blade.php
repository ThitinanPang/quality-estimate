...
@section('content')
    <p class="absolute w-[330px] h-[53px] left-[85px] top-[200px] font-normal text-[40px] leading-[54px]">
        ข้อมูลผู้ดูแลระดับคณะ</p>
    {{-- ตัวอย่าง excel --}}
    <a href="{{ route('users.template') }}"
        class="absolute left-[330px] text-[20px] top-[210px] w-[174px] h-[30px] gap-2 border rounded-[10px] flex items-center justify-center bg-[#FFCE00]">
    ...
    <div class="flex">
        <button onclick="window.location.href='{{route('user')}}'"
            class="w-[135px] h-[38px] rounded-[5px] bg-[#D9D9D9] hover:bg-[#BEBEBE] text-[24px] ml-[85px] mt-[150px]">ผู้ใช้ทั่วไป</button>
        <button onclick="window.location.href='{{route('faculty')}}'"
            class="{{ request()->routeIs('faculty') ? 'bg-[#BEBEBE]' : 'hover:bg-[#D9D9D9]' }} w-[233px] h-[38px] rounded-[5px] hover:bg-[#BEBEBE] text-[24px] ml-[10px] mt-[150px]">ผู้ดูแลระดับคณะ</button>
    ...
    <div class="overflow-x-auto flex items-center justify-center mt-[20px]">
        <table id="myTable" class="w-[1350px] border border-black box-border" style="width:1350px;">
            ...
            <tbody>
                ...
                @else
                    <tr>
                        <td class="px-4 py-2 border-b text-center text-[20px]" colspan="8">ไม่มีข้อมูล</td>
                    </tr>
                @endif
            </tbody>
        </table>
